...
while iteration < FLAGS.max_iter:
  _, g_loss_curr, g_mhe_loss_curr = sess.run([g_solver, g_loss, g_mhe_loss], feed_dict={z: generator.generate_noise(), is_training: True})
  for _ in range(FLAGS.n_dis):
    _, d_loss_curr, d_mhe_loss_curr, summaries = sess.run([d_solver, d_loss, d_mhe_loss, merged_summary_op],
  ...
  if (iteration + 1) % FLAGS.display_interval == 0 and not is_start_iteration:
    ...
    print('Iter {}: d_loss = {:4f}, g_loss = {:4f}, d_mhe = {:4f}, g_mhe = {:4f}, time = {:2f}s'.format(
      iteration, d_loss_curr, g_loss_curr, d_mhe_loss_curr, g_mhe_loss_curr, stop - start))
    start = stop
  ...
  if (iteration + 1) % FLAGS.evaluation_interval == 0:
    ...
    if inception_score_mean > best_inception:
      ...
      best_saver.save(sess, 'snapshots/best_model.ckpt')
    inception_scores.append(dict(iteration=iteration, mean=inception_score_mean, std=inception_score_std))
